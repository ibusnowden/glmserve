...
| OpenAI-compatible server: `/v1/chat/completions` (+SSE), `/v1/completions`, `/v1/models`, `/health` | ✅ done |
...
| GLM DSA indexer weights | ✅ loader/checker recognize tensors; 🚧 not active in sparse key selection |
...
| **TP weight-sharding in the forward** (head-sharded MLA, gate/up column- + down row-parallel MLP/MoE, all-reduce) | ✅ TP=2/PP=1 forward matches single-process to **1.5e-7** (2× RTX, job 125290) |
| GPU-path TP/PP, multi-process serving driver, learned DSA indexer, MTP | 🚧 roadmap |
...
- **Proven on the tiny checkpoint** (weight *values* don't matter for these):
  ...
  the bit-exact comparison runs on 2 GPUs in under a minute.
...
- 🚧 **Phase 5c** the CPU-reference TP/PP path on the device-resident GPU forward + a multi-process serving driver (launch ranks, route tokens/logits through the HTTP front-end)
- 🚧 **Phase 6–7** fused MoE grouped-GEMM, learned DSA top-k indexer execution, MTP speculative decoding, prefix cache
